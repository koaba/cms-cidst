<x-admin.layout> 
    ...
    <form method="POST" action="{{ route('admin.articles.update', $article) }}" enctype="multipart/form-data" class="bg-white shadow rounded p-6 max-w-3xl">
        ...
        <div class="mb-6 border-t pt-4">
            ...
            <label class="flex items-center gap-2 text-sm text-gray-700 mt-2">
                <input type="checkbox" name="apply_watermark_diaporamas" value="1">
                Appliquer le filigrane de protection sur les images des diaporamas
            </label>
        </div> 
        ...
        function addVideo() { 
            ...
            wrapper.innerHTML = `
                ...
                <div class="flex gap-4 mb-2 text-sm">
                    <label class="flex items-center gap-1">
                        <input type="radio" name="videos[${index}][source_type]" value="upload" checked onchange="toggleVideoSource(${index}, 'upload')">                        Fichier vidéo
                    </label>
                    <label class="flex items-center gap-1">
