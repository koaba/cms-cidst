<x-admin.layout>
    ...
    <form action="{{ route('admin.news-tickers.update', $newsTicker) }}" method="POST" class="max-w-lg space-y-4">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea name="content" rows="3" required
                      class="w-full border rounded p-2">{{ old('content', $newsTicker->content) }}</textarea>
            @error('content') <p class="text-red-600 text-sm">{{ $message }}</p> @enderror
        </div>

        <div>
            <label class="block font-medium mb-1">Lien (optionnel)</label>
            <input type="url" name="link_url" value="{{ old('link_url', $newsTicker->link_url) }}"
        ...
        <div>
            ...
            <input type="number" name="order" value="{{ old('order', $newsTicker->order) }}"
                   class="w-full border rounded p-2">
        </div>

        <div class="flex items-center gap-2">
            <input type="checkbox" name="is_active" id="is_active" value="1"
                   {{ old('is_active', $newsTicker->is_active) ? 'checked' : '' }}>
